...
from .forms import CreatefieldsForm, CreatedefaultForm, CreatetypefilterForm, CreateTDForm
import csv
...
class lensDetailView(generic.DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        fields_td = ["Name","BibCode_TD", "tAB","ErrtAB","tAC","ErrtAC","tBC","ErrtBC","tAD","ErrtAD","tBD","ErrtBD","tCD","ErrtCD"]
        table_td, lens_id = create_table_pd(fields_td, name=name)
        filtered_td = [[],[]]
        for i in range(len(table_td[0])):
            if(table_td[1][i] != ""):
                filtered_td[0].append(table_td[0][i])
                filtered_td[1].append(table_td[1][i])
        print(filtered_td)
        ...
        if(len(filtered_td[0]) == 0):
            context['has_td'] = False
        else:
            context['has_td'] = True
        context['table_td'] = filtered_td
...
def create_table_pd(fields, filter= None, name = False):
    ...
    if(filter != None):
        ...
        if(filter_type == "doubles"):
            merged_df = merged_df[merged_df["Type"] == "Double"]
        elif(filter_type == "quads"):
            merged_df = merged_df[merged_df["Type"] == "Quad"]
...
def paginate(table, start_row, n_rows):
    new_table = [table[0]]
    table_length = len(table)
    if (start_row + n_rows < table_length-1):
        showing = f"{start_row + 1}-{start_row + n_rows +1}"
        table = table[start_row+1:start_row + n_rows+1]
    else:
        showing = f"{start_row + 1}-{table_length}"
        table = table[start_row+1:]
    for column in table:
        new_table.append(column)
    return new_table, showing
...
def lenses(request):
    ...
    #defaultlist = ["Name", "RA_center", "DEC_center","RA_center_sexa", "DEC_center_sexa", "Type", "BibCode", "Max_separation", "z_source", 'z_deflector_text', "z_bibcode"]
    ...
    tdfilterform = CreateTDForm()
    tdfilterform_values = request.GET.getlist('tdfilterform')
    print(tdfilterform_values)
    ...
    if(request.headers.get('x-requested-with') == 'defaultform'):
        #print("lolll", defaultform_values[0])
        if(defaultform_values[0] == "default"):
            savedfields = defaultlist
        elif(defaultform_values[0] == "all"):
            savedfields = fields
        print("nah", savedfields)
    else:   
    ...
    if(len(typefilterform_values)>0):
        filter_dic["type"] = typefilterform_values[0]
    if(len(tdfilterform_values)>0):
        filter_dic["td"] = tdfilterform_values[0]
    print(filter_dic)
    table, lens_id = create_table_pd(savedfields, filter_dic)
    ...
    headers = request.headers.get('x-requested-with')
    n_rows = 50
    try:
        previous_start_row = int(request.GET.get('start_row', 0))
        print(previous_start_row)
    ...
    if(headers == "next"):
        if(start_row+n_rows < table_length):
            start_row += n_rows
    if(headers == "back"):
        if(start_row < n_rows):
            start_row = 0
            ...
            start_row -= n_rows
    print(headers, start_row, n_rows)
    print(table_length)
    table, showing = paginate(table, start_row, n_rows)
    if(headers != None):
...
def components(request):
    ...
    #components = LensComponent.objects.values()
    #lenses = Lens.objects.values()
    defaultlistcomp = ["Name", "Component","RA_best", "DEC_best","ra_sexa", "dec_sexa", "Type", "BibCode", "Max_separation", "z_source", 'z_deflector', "z_bibcode"]
